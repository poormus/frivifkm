...
class ViewSeenByMembers extends StatelessWidget {
  final List<String> memberUids;
  final String organizationId;
  const ViewSeenByMembers({Key? key, required this.memberUids, required this.organizationId}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final provider = Provider.of<MyProvider>(context);
    final size=MediaQuery.of(context).size;
    return BaseScaffold(appBarName: 'Seen by'.tr(), body: buildBody(provider,size), shouldScroll: false);
  ...
  Widget buildBody(MyProvider provider,Size size){
    ...
    return userList.length==0?noDataWidget('No Data Found', false):ListView.builder(
