...
class ViewGroupMembers extends StatelessWidget {
  ...
  final Group group;
  const ViewGroupMembers({Key? key, required this.memberUids, required this.group}) : super(key: key);
  ...
  List<CurrentUserData> getUsersForGroup(MyProvider provider) {
    ...
    provider
        .getCurrentOrganizationUserList(group.organizationId)
        .forEach((element) {
      if (group.uidList.contains(element.uid)) {
